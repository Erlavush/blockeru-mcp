...
import { applySemanticRepairHints } from "./semanticPlanRepair.js";
...
export type RepairPlanningHints = {
  ...
  addArmrests: boolean;
  addSideSlats: boolean;
  addFrontBeam: boolean;
  addSeatCushion: boolean;
  addBackCushion: boolean;
  insetSeat: boolean;
  raiseBack: boolean;
  completeBedFrame: boolean;
  closeBedSides: boolean;
  closeBedUnderside: boolean;
  embedDrawers: boolean;
};
...
function deriveMutationScope(projectName: string, assetType: string): string {
...
function applyMutationScope(cubes: PlannedCube[], mutationScope: string): PlannedCube[] {
  return cubes.map((cube) => ({
    ...cube,
    name: `${mutationScope}::${cube.name}`,
  }));
}
...
function pickMaterialSlots(
  ...
  const primaryColor = spec.materialColorHints[primaryMaterial] ?? spec.palette[0] ?? "natural";
  const secondaryColor =
    spec.materialColorHints[secondaryMaterial] ??
    spec.palette[1] ??
    spec.palette[0] ??
    primaryColor;
  const accentColor = spec.palette[2] ?? spec.palette[1] ?? spec.palette[0] ?? "natural";
  ...
  return resolvedSlotIds.map((slotId) => {
    ...
    switch (slotId) {
      case "secondary":
        return makeSlot(slotId, "Secondary", secondaryMaterial, secondaryColor, placement);
      case "accent":
        return makeSlot(slotId, "Accent", accentColor, accentColor, placement);
      case "neutral":
        return makeSlot(slotId, "Neutral", "neutral", "gray", placement);
      default:
        return makeSlot(slotId, "Primary", primaryMaterial, primaryColor, placement);
    }
  });
}
...
export function planBuildFromAssetSpec(options: {
  ...
  const repairedDraftCubes = normalizeScenePlacement(applySemanticRepairHints({
    cubes: normalizeScenePlacement(
    ...
    repairHints,
  }), options.formatId, repairHints);
  const draftCubes = repairedDraftCubes;
  ...
  const mutationScope = deriveMutationScope(projectName, spec.assetType);
  const scopedCubes = applyMutationScope(cubes, mutationScope);

  return {
    projectName,
    mutationScope,
    managedTextureName: `${mutationScope}__atlas`,
    ...
    notes: [
      ...
      `Generated cubes are namespaced under the managed mutation scope "${mutationScope}" for revision-safe patch updates.`,
      options.boxUv
      ...
      repairHints.addArmrests || repairHints.addSideSlats || repairHints.addFrontBeam
        ? "Semantic repair pass added frame features based on reference intent findings."
        : "No semantic frame repairs were applied.",
      repairHints.addSeatCushion || repairHints.addBackCushion || repairHints.insetSeat || repairHints.raiseBack
        ? "Semantic repair pass adjusted cushions/back relationships against reference intent."
        : "No semantic cushion/back repairs were applied.",
      repairHints.completeBedFrame ||
      repairHints.closeBedSides ||
      repairHints.closeBedUnderside ||
      repairHints.embedDrawers
        ? "Semantic repair pass completed hidden bed structure and closed open storage geometry."
        : "No hidden bed-structure completion repairs were applied.",
    ],
  };
}
